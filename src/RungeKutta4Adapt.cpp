...
#include "RungeKutta4Adapt.h"
#include "MenuSystem.h"

RungeKutta4Adapt::RungeKutta4Adapt(ODEProblem* eqdef) : ODESolver (eqdef) {}

void RungeKutta4Adapt::init() {
  _scratch1.resize(_eqdef->size());
  _scratch2.resize(_eqdef->size());
  _scratch3.resize(_eqdef->size());
  _scratch4.resize(_eqdef->size());
  initFromCommandLine("-tol", _tol, 1e-3);
  initFromCommandLine("-ec", _ec, 1e-3);
}

void RungeKutta4Adapt::advance(vectorr64& v, r64& t, r64& dt) {
  u32 n = v.size()  ;
  r64 dt2 = dt * 0.5;
  r64 dt3 = dt / 3.0;
  r64 dt6 = dt3 * 0.5;
